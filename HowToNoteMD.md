...
来源：https://support.typora.io/zh/Markdown-Reference/
...
输入 `* list item 1` 将创建一个**无序列表**，该 `*` 符号可以替换为 `+` 或 `-`.

输入 `1. list item 1` 将创建一个**有序列表**，其 markdown 源代码如下：
...
> 🌟关于公式输入的详细教程可以翻看：[♏️怎样输入公式♏️](HowToTypeFormula.md)
...
最后，通过==在标题行中包含冒号：您可以将文本定义为左对齐，右对齐或居中对齐==：
...
Command(在Windows上：Ctrl) + 单击 [此链接](###block 元素) 将跳转到标题 `block 元素`处。 要查看如何编写，请移动光标或按住 `⌘`键单击以将元素展开为 Markdown 源代码。
...
要触发内联公式的==预览提示功能：输入“$”, 然后按 `ESC` 键==, 然后输入TeX命令, 预览工具提示
