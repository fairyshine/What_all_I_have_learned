...
第一，LM-Language Model，语言模型，自回归（从左向右）的基于马尔可夫假设的（目前的事实，只和之前的和历史上的事实相关，和未来的无关）。是一般在句子上建模，把一个序列的概率按照各个词的概率进行拆解；例如，p(I love you) = p(I|<s>)p(love|I)p(you|I, love)。
...
第四，PLM-permuted language model，(全)排列语言模型，基于一个新的排列（可能是随机生成的），然后在该排列的指导下，构建自回归形式下的概率分解。例如I love you，假设有个全排列是 2 1 3，即新的序列是love I you，那么概率就是p(love|<s>)p(I|love)p(you|love,I)。
...
# 3 NLP算法面试必备！PTMs：NLP预训练模型的全面总结
...
Context Based基本可以理解成Generative Pre-Training

Contrastive Based则是Contrastive Pre-training

![PTMs：NLP预训练模型的全面总结](pic/PTMs：NLP预训练模型的全面总结.jpg)

 [NLP算法面试必备！PTMs：NLP预训练模型的全面总结 - 知乎.pdf](resources/NLP算法面试必备！PTMs：NLP预训练模型的全面总结 - 知乎.pdf) 

# 4 生成式(预测式)  or  对比式——的预训练
...
生成式无监督预训练简介（Generative Pre-Training）
